...
import { Hive } from '../hive/hive.model.js';
import { Batch } from '../harvest/batch.model.js';
import { Inspection } from '../inspection/inspection.model.js';
import { Farmer } from '../farmer/farmer.model.js';
import { Request } from '../request/request.model.js';
import { SavedLocation } from '../location/savedLocation.model.js';
...
const SAFE_FIELDS = {
  hives:          ['hive_id', 'lat', 'lng', 'box_count', 'queen_status', 'health_status', 'last_inspection_date', 'notes', 'syncVersion', 'uid', 'placement_location_id', 'placement_location_name'],
...
async function processSingleOp(op, user) {
  try {
    ...
    // Align flat lat/lng to GeoJSON required by SavedLocation model
    if (op.entity === 'savedLocations' && cleanData.lat !== undefined && cleanData.lng !== undefined) {
      cleanData.location = { type: 'Point', coordinates: [cleanData.lng, cleanData.lat] };
