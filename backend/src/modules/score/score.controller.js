...
import { computeWeatherScore, computeFloraScore, computeSeasonScore, deriveOutputs } from './scoring.service.js';
...
export async function scoreController(req, res, next) {
  try {
    ...
    try {
      ...
      console.warn('[Score] ML service unavailable, falling back to deterministic scoring:', err.message);
      // Fallback: Compute a reasonable score based on sub-scores
      const wScore = computeWeatherScore(weatherData.avgTemp, weatherData.avgRain, weatherData.avgWind);
      const fScore = computeFloraScore(floraData.floraCount);
      const sScore = computeSeasonScore(month);
      
      // Weight them similar to ML importance
      const fallbackScore = Math.round((wScore * 0.35) + (fScore * 0.40) + (sScore * 0.25));
      
      mlResponse = {
        score: fallbackScore,
        confidence: 0.65, // Lower confidence for fallback
        risk: fallbackScore >= 60 ? 'Low' : (fallbackScore >= 35 ? 'Moderate' : 'High'),
        flowering_level: fallbackScore >= 60 ? 'High' : (fallbackScore >= 35 ? 'Medium' : 'Low')
      };
    ...
    payload.mlModel = dataSource === 'live_inference' ? 'RandomForest_Live' : 'RuleEngine_v1';
