...
export function computeFinalScore(weatherScore, floraScore, seasonScore, weights = { weather: 0.35, flora: 0.40, season: 0.25 }) {
  // Ensure weights sum to 1.0 (if ML service returns non-normalized coefficients)
  const totalWeight = (weights.weather || 0) + (weights.flora || 0) + (weights.season || 0);
  const wWeather = (weights.weather || 0) / totalWeight;
  const wFlora = (weights.flora || 0) / totalWeight;
  const wSeason = (weights.season || 0) / totalWeight;

  const rawScore = (wWeather * weatherScore) + (wFlora * floraScore) + (wSeason * seasonScore);
