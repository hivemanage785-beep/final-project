...
import { X, MapPin, Trash2, BarChart2, Loader2, Navigation, Zap } from 'lucide-react';
      ...
      <div className="p-4 border-t border-gray-100 bg-slate-50">
        {!canRunBulk ? (
          <div className="text-center py-2">
             <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Candidate Pool Too Small</p>
             <p className="text-[10px] text-slate-400 mt-1">Save {2 - locations.length} more locations to run simulations</p>
          </div>
        ) : (
          <button
            onClick={onOptimize}
            className="w-full flex items-center justify-center gap-2 py-4 rounded-2xl bg-[#5D0623] text-white hover:bg-[#4a051c] active:scale-[0.98] transition-all shadow-lg shadow-rose-900/20 text-sm font-bold"
          >
